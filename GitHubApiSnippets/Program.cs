...
namespace GitHubApiSnippets
{
	...
	{
		private static readonly string GitHubIdentity = //"gayeetri-naik_psl";
		Assembly
		.GetEntryAssembly()
		...
		{
            //Repository repository = await client.Repository.Get("octokit", "octokit.net");
            Repository repository = await client.Repository.Get("namitamehta", "engineer-ai-Test");
            Console.WriteLine($"Repository.Get: Id={repository.Id}");

			var commit = await client.Repository.Commit.GetAll("namitamehta", "SampleRepToLearn");
            //Branch branch = await client.Repository.Branch.Get("octokit", "octokit.net", "master");
            Branch branch = await client.Repository.Branch.Get("namitamehta", "engineer-ai-Test", "master");
            Console.WriteLine($"Repository.Branch.Get: Name={branch.Name}");

			SearchRepositoryResult result = await client.Search.SearchRepo(
				new SearchRepositoriesRequest("engineer-ai-Test")
				{
